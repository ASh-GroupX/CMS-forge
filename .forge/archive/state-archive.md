...
## F7-02B Built - AUTO PHASE Stopped At Planner Split

`F7-02B` added guarded `GET /complaints/search` with session auth, RBAC,
branch-scope guard metadata, safe staff search response DTOs, bounded
`limit`/`offset` pagination, and canonical OpenAPI. Non-admin branch scope comes
from `request.principal.branchId`; query branch filters cannot widen scoped users.

Required proof passed: lint, typecheck, test 31/31, test:api -- search 4/4,
openapi:check, and git diff --check with line-ending warnings only. F7-02 is now
complete. AUTO PHASE stops here because F7-03A spans real login/session
authority, cookie forwarding, `/auth/me`, and removal of `?role=` authority; it
needs PLANNER splitting before a safe direct build.

## F7-02A Built - AUTO PHASE Continuing To F7-02B

`F7-02A` added a branch-scoped complaint search read model inside the existing
complaints service/repository boundary. Filters cover reference number, customer
name/phone/identifier, status, severity, owner, and date range. Search returns
safe staff read fields only and adds no route, OpenAPI path, frontend change,
schema/migration, provider call, external system call, or portal response.

Required proof passed after one honest repair: the first `test:api -- workflow`
run exposed a repository syntax error, and the first `lint` run found the touched
service/repository files over the 300-line source budget. Both were fixed. Final
proof passed: lint, typecheck, test 31/31, test:api -- workflow 39/39,
openapi:check, and git diff --check with line-ending warnings only. AUTO PHASE
continues with `F7-02B`.
