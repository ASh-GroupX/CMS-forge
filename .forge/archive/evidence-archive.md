...
- Requirement IDs:
  ...
  - METHOD-MODULAR-001
- Evidence:
  - Extended the existing complaints read model instead of adding a parallel
    search module.
  - Added `ComplaintsService.search(...)` backed by the complaints repository.
  - Search filters cover reference number, customer name/phone/identifier,
    status, severity, owner, and created date range.
  - Branch scope remains a service/repository input for the later guarded route
    to derive from the server session.
  - Search rows expose safe staff read fields only: queue fields, category,
    customer name, customer phone, and customer identifier.
  - Added no HTTP route, OpenAPI path, frontend change, schema/migration,
    provider call, external system call, or portal response.
  - Tests cover a scoped allowed result and a hidden out-of-branch result.
- Verification:
  - Passed: `corepack pnpm lint`
  - Passed: `corepack pnpm typecheck`
  - Passed: `corepack pnpm test` (31/31)
  - Passed: `corepack pnpm test:api -- workflow` (39/39)
  - Passed: `corepack pnpm openapi:check`
  - Passed: `git diff --check` (line-ending warnings only)
- Security Self-Check:
  - Roles and branch scope come from the server session, never client input:
    Passed by design and scope. F7-02A adds no route; branch scope is an explicit
    service/repository input for F7-02B's guarded route to derive from
    `request.principal`.
  - Each state change writes status history and an audit entry in the same
    transaction; side effects enqueue after commit: Passed by scope. This task is
    read-only and adds no state change, audit-bearing mutation, or side effect.
  - No passwords, OTPs, tokens, hashes, or provider secrets are logged or
    returned: Passed. Search returns safe complaint/customer read fields only and
    adds no logging, secret access, or provider call.
  - Customer portal exposure rules hold: Passed by scope. No portal route or
    portal response was added; search does not return internal comments, audit
    logs, DMS codes beyond staff-only customer identifier, staff PII, or portal
    verification/session data.
  - Trust boundaries are tested: Passed. `test:api -- workflow` covers an allowed
    scoped search result and a hidden out-of-branch result; `lint`/`test` also run
    manifest truth and module wiring gates.
...
- Requirement IDs:
  ...
  - METHOD-MODULAR-001
- Evidence:
  - Added guarded `GET /complaints/search`.
  - Route uses existing `SessionAuthGuard`, `RbacGuard`, `@Roles`, and
    `@BranchScoped`.
  - Non-admin branch scope is derived from `request.principal.branchId`; query
    branch filters cannot widen scoped users.
  - Query filters cover reference number, customer query, status, severity,
    owner, and date range.
  - Added bounded pagination: default `limit=25`, maximum `limit=100`, default
    `offset=0`, and invalid numeric/enumerated query values reject with
    `VALIDATION_FAILED`.
  - Added `ComplaintSearchItem`/`ComplaintSearchResponse` response types and
    canonical OpenAPI JSON for the route and schemas.
  - Added `test:api -- search` runner support and focused route tests covering
    allowed scoped search, hidden out-of-branch search, audited branch-scope
    denial, and invalid pagination/enums.
  - Added no frontend change, schema/migration, provider call, portal route, or
    portal response.
- Verification:
  - Passed: `corepack pnpm lint`
  - Passed: `corepack pnpm typecheck`
...
- Security Self-Check:
  - Roles and branch scope come from the server session, never client input:
    Passed. The route derives non-admin branch scope from `request.principal` and
    uses existing session/RBAC/branch-scope guards; tests cover guarded
    branch-scope denial and controller-level non-widening.
  - Each state change writes status history and an audit entry in the same
    transaction; side effects enqueue after commit: Passed by scope. This task
    adds a read-only route and no state change, audit-bearing mutation, or side
    effect.
  - No passwords, OTPs, tokens, hashes, or provider secrets are logged or
    returned: Passed. The route returns safe staff complaint search fields only
    and adds no logging, secret access, provider call, or browser storage.
  - Customer portal exposure rules hold: Passed by scope. This is a guarded staff
    route and adds no portal route/response; it does not return internal
    comments, audit logs, DMS codes to the portal, staff PII, or portal
    verification/session data.
  - Trust boundaries are tested: Passed. `test:api -- search` covers allowed
    scoped search, hidden out-of-branch search, and audited branch-scope denial;
    `lint`/`test` also run manifest truth, wiring, and OpenAPI gates.
