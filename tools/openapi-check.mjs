...
const file = 'packages/contracts/openapi.json', eventTypes = ['AUTH', 'USER_ADMIN', 'COMPLAINT', 'WORKFLOW', 'COMMENT', 'ATTACHMENT', 'SLA', 'NOTIFICATION', 'REPORT', 'CONFIG', 'SECURITY'];
...
const canonical = {
  ...
  paths: {
    ...
    '/complaints/search': { get: { tags: ['Complaints'], operationId: 'complaintSearch', summary: 'Search branch-scoped staff complaints', parameters: [param('branchId'), param('referenceNumber'), param('customer'), param('status', { type: 'string', enum: complaintStatuses }), param('severity', { type: 'string', enum: complaintSeverities }), param('ownerId'), param('dateFrom', { type: 'string', format: 'date-time' }), param('dateTo', { type: 'string', format: 'date-time' }), param('limit', { type: 'integer', minimum: 1, maximum: 100 }), param('offset', { type: 'integer', minimum: 0 })], responses: { 200: ok('Complaint search results', ref('ComplaintSearchResponse')), 400: error('Invalid query'), 401: error('Missing or invalid session'), 403: error('Role or branch scope denied') } } },
    '/complaints': {
      get: {
        tags: ['Complaints'],
  ...
  components: {
    schemas: {
      ...
      ComplaintSearchItem: object(['id', 'referenceNumber', 'status', 'severity', 'subject', 'branchId', 'ownerId', 'createdAt', 'updatedAt', 'categoryId', 'customerName', 'customerPhone', 'customerIdentifier'], { id: str, referenceNumber: str, status: { type: 'string', enum: complaintStatuses }, severity: { type: 'string', enum: complaintSeverities }, subject: str, branchId: str, ownerId: { type: ['string', 'null'] }, createdAt: { type: 'string', format: 'date-time' }, updatedAt: { type: 'string', format: 'date-time' }, categoryId: str, customerName: str, customerPhone: str, customerIdentifier: { type: ['string', 'null'] } }),
      ComplaintSearchResponse: object(['items', 'limit', 'offset'], { items: { type: 'array', items: ref('ComplaintSearchItem') }, limit: { type: 'integer', minimum: 1, maximum: 100 }, offset: { type: 'integer', minimum: 0 } }),
