...
const REPORT_CATALOG: readonly ReportCatalogItem[] = [
  ...
  deferred('RPT-002', 'Overdue complaints', 'Managers, Management', 'YES', ['branch', 'department', 'owner', 'severity', 'SLA stage'], ['complaint list', 'overdue duration', 'current owner'], ['Dashboard overdue count and filtered operational rows exist.'], ['Specialized overdue list with overdue duration, SLA stage filter, and current-owner output is deferred pending signed business scope.']),
  deferred('RPT-003', 'SLA warning complaints', 'Managers, CR team', 'YES', ['branch', 'owner', 'severity'], ['complaint list', 'percent elapsed', 'deadline'], ['Dashboard SLA warning count exists.'], ['Specialized warning list with percent elapsed and deadline output is deferred pending signed business scope.']),
  ...
  deferred('RPT-005', 'Closure performance by branch', 'Management', 'YES', ['date', 'branch'], ['created', 'closed', 'overdue', 'breach rate', 'avg TAT'], ['Branch filtering plus closed, overdue, breach-rate, and TAT formulas exist.'], ['Branch-grouped performance output is deferred pending signed business scope.']),
  deferred('RPT-006', 'Complaints by category', 'Management, CR Manager', 'YES', ['date', 'branch', 'department', 'severity'], ['count', 'percentage', 'trend'], ['Category filtering exists on operational report rows.'], ['Category-grouped percentages and trend output are deferred pending signed business scope.']),
  deferred('RPT-007', 'Complaints by brand/model', 'Management', 'YES', ['date', 'brand', 'model', 'branch', 'department'], ['count', 'rate if denominator exists', 'trend'], ['Vehicle provenance and DMS lookup foundations exist.'], ['Brand/model report output and denominator policy are deferred pending signed business scope.']),
  deferred('RPT-008', 'Complaints by department', 'Management', 'YES', ['date', 'branch', 'department'], ['count', 'open', 'closed', 'overdue', 'avg TAT'], ['Department filtering exists on operational report rows.'], ['Department-grouped counts and TAT output are deferred pending signed business scope.']),
  deferred('RPT-009', 'Owner workload', 'CR Manager, Branch Manager', 'YES', ['branch', 'department', 'owner', 'status'], ['assigned', 'overdue', 'closed', 'avg handling time'], ['Owner filtering exists on operational report rows.'], ['Owner workload aggregation is deferred pending signed business scope.']),
  deferred('RPT-010', 'Reopened complaints', 'Management, CR Manager', 'YES', ['date', 'branch', 'category', 'owner'], ['count', 'reopen rate', 'reason list'], ['Reopen count and reopen-rate KPI formulas are implemented.'], ['Reason-list output is deferred pending signed business scope.']),
  deferred('RPT-011', 'Rejected complaints', 'CR Manager', 'YES', ['date', 'branch', 'rejection reason'], ['count', 'list', 'reason summary'], ['Rejected complaints are available as complaint statuses in scoped rows.'], ['Rejected report count/list/reason summary is deferred pending signed business scope.']),
