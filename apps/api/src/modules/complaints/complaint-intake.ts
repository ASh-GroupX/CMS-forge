...
import type { ComplaintRecord, CreateComplaintData } from './complaints.repository.js';
...
export function createComplaintData(input: CreateInternalComplaintInput): Omit<CreateComplaintData, 'referenceNumber'> {
  const errors = [
    ...
    ...(input.vehicleRelated && !optionalText(input.vehicleId) ? requiredTextError(input.vehicleVin, 'vehicleVin') : []),
  ];
...
export function complaintCreatedAudit(input: CreateInternalComplaintInput, complaint: ComplaintRecord): AuditRecordInput {
  return {
    ...
    metadata: { referenceNumber: customerReference(complaint.referenceNumber), status: complaint.status, severity: complaint.severity },
  };
}
