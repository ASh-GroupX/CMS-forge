...
function slaSnapshot(complaint: Pick<ComplaintQueueRecord, 'createdAt' | 'severity' | 'status'>, slaService?: SlaService): Pick<ComplaintQueueItemDto, 'slaDueAt' | 'slaPercentElapsed' | 'slaStage' | 'slaState'> {
  ...
  const deadline = slaService.calculateDeadline({ severity: complaint.severity, stage: SlaStage.RESOLUTION, durationMinutes, warningPercent: 80, branchTimezone: 'UTC', workingCalendarMode: WorkingCalendarMode.ALWAYS_ON, enteredAt: complaint.createdAt });
