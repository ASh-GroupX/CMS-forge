  ...
  return (
    <section className="grid gap-4" aria-label={t.title}>
      <header className="grid gap-3 rounded-md border border-line-subtle bg-surface-raised p-4">
        ...
        <dl className="grid gap-3 text-sm sm:grid-cols-3"><Fact label={t.responsible} value={task.assigneeName ?? missingDisplay(locale)} /><Fact label={t.due} value={`${formatDisplayDate(task.dueAt, locale, { dateStyle: 'medium', timeStyle: 'short', timeZone: task.displayTimeZone })} (${task.displayTimeZone})`} /><Fact label={t.next} value={task.nextAction?.what ?? missingDisplay(locale)} /></dl>
        {complaintId ? <a className="w-fit text-sm font-medium text-brand underline-offset-4 hover:underline" href={localeHref(`/complaints/${complaintId}?tab=communication`, locale)}>{locale === 'ar' ? 'فتح الشكوى المرتبطة' : 'Open linked complaint'}</a> : null}
      </header>
      <section className="grid gap-2 rounded-md border border-line-subtle bg-surface-raised p-4"><h2 className="text-sm font-semibold">{t.comments}</h2>{items.length ? <ol className="grid gap-2">{items.map((comment) => <TaskComment comment={comment} key={comment.id} locale={locale} timeZone={task.displayTimeZone} />)}</ol> : <StateBlock message={t.empty} />}</section>
      <form className="grid gap-4 rounded-md border border-line-subtle bg-surface-raised p-4" onSubmit={submit}>
        ...
        {showAdditions ? <div className="rounded-md border border-line-subtle bg-surface p-3"><AudiencePicker cc={cc} disabled={!canComment || status === 'loading'} key={pickerVersion} loadTargets={loadTargets} locale={locale} mentions={mentions} onLoaded={loaded} onRemoveWatcher={removeWatcher} setCc={setCc} setMentions={setMentions} /></div> : null}
        <p className="text-sm text-content-muted" role="status">{collaboration.internalAudience.replace('{mentions}', String(mentions.length)).replace('{cc}', String(cc.length))}</p>
