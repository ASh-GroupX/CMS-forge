...
function SelectedTargets({ locale, onRemove, selections }: { locale: Locale; onRemove: (kind: 'mention' | 'cc', target: CollaborationTarget) => void; selections: { kind: 'mention' | 'cc'; target: CollaborationTarget }[] }) {
  ...
  return <div className="grid gap-2" aria-label={t.selected}><p className="text-sm font-medium">{t.selected}</p><div className="flex flex-wrap gap-2">{selections.map(({ kind, target }) => <span className="inline-flex max-w-full items-center gap-1 rounded-sm border border-line-subtle bg-surface px-2 py-1 text-sm" key={`${kind}:${target.type}:${target.id}`}><span className="truncate">{kind === 'mention' ? '@' : t.cc}: <bdi>{display(target, locale)}</bdi></span><Button aria-label={`${t.remove} ${display(target, locale)}`} className="size-11 p-0" onClick={() => onRemove(kind, target)} title={t.remove} type="button" variant="ghost"><X aria-hidden="true" className="size-4" /></Button></span>)}</div></div>;
}
...
function context(target: CollaborationTarget, locale: Locale): string { return [locale === 'ar' ? target.roleNameAr || target.roleName : target.roleName || target.roleNameAr, locale === 'ar' ? target.departmentNameAr || target.departmentName : target.departmentName || target.departmentNameAr, locale === 'ar' ? target.branchNameAr || target.branchName : target.branchName || target.branchNameAr].filter(Boolean).join(' · '); }
