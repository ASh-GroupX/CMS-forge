...
import { formatDisplayDate, formatDisplayNumber, missingDisplay, zonedDateTimeToIso } from '../../lib/locale-format';
...
export function ComplaintCommentsPanel({ comments, commentsState, complaintId, initialVisibility = 'INTERNAL', locale, timeZone = 'UTC' }: { comments?: StaffComplaintComment[] | null | undefined; commentsState?: ComplaintCommentsFixtureState | undefined; complaintId?: string | undefined; initialVisibility?: StaffComplaintCommentVisibility | undefined; locale: Locale; timeZone?: string }) {
  ...
  async function send(confirmedRecipientCount?: number) {
    ...
    const result = await addStaffComplaintComment(complaintId, {
      ...
      ...(visibility === 'INTERNAL' ? {
        ...
        ...(actionMode === 'task' && assignee ? { actionTask: { title: actionTitle.trim(), assigneeId: assignee.id, dueAt: zonedDateTimeToIso(dueAt, timeZone) } } : {}),
      } : {}),
    });
    if (!result.ok) {
  ...
  return (
    <section className="grid gap-4" aria-label={`${detail.sections.internalComments} / ${detail.sections.publicUpdates}`}>
      {commentsState ? <StateBlock message={detail.commentStates[commentsState]} tone={commentsState === 'error' ? 'error' : 'neutral'} /> : <>
        <CommentGroup badge={detail.badges.internal} comments={grouped.internal} locale={locale} timeZone={timeZone} title={detail.sections.internalComments} />
        <CommentGroup badge={detail.badges.public} comments={grouped.public} locale={locale} timeZone={timeZone} title={detail.sections.publicUpdates} />
        <form aria-label={copy.form} className="grid gap-4 rounded-md border border-line-subtle bg-surface-raised p-4" onSubmit={submit}>
          ...
          {visibility === 'INTERNAL' ? <>
            <Button className="w-fit" onClick={() => setShowAdditions((value) => !value)} type="button" variant="outline">{showAdditions ? collaboration.hideAdditions : collaboration.additions}</Button>
            {showAdditions ? <div className="grid gap-4 rounded-md border border-line-subtle bg-surface p-3">
              ...
              {actionMode === 'task' ? <ActionTaskFields assignee={assignee} dueAt={dueAt} locale={locale} onDueAtChange={setDueAt} onTitleChange={setActionTitle} timeZone={timeZone} title={actionTitle} /> : null}
            </div> : null}
          </> : null}
          {visibility === 'INTERNAL' ? <p className="text-sm text-content-muted" role="status">{audienceSummary(collaboration.internalAudience, mentions.length, cc.length)}</p> : null}
      ...
      <ActionDialog description={collaboration.audienceConfirmDescription} footer={<><Button onClick={() => setAudienceConfirm(null)} type="button" variant="outline">{collaboration.cancel}</Button><Button onClick={() => { const count = audienceConfirm; setAudienceConfirm(null); if (count !== null) void send(count); }} type="button">{collaboration.audienceConfirmAction}</Button></>} onOpenChange={(open) => { if (!open) setAudienceConfirm(null); }} open={audienceConfirm !== null} title={fill(collaboration.audienceConfirmTitle, formatDisplayNumber(audienceConfirm, locale))}><p className="text-sm font-semibold">{fill(collaboration.audienceConfirmTitle, formatDisplayNumber(audienceConfirm, locale))}</p><p className="mt-2 text-sm text-content-muted">{collaboration.selected}: <bdi>{[...mentions, ...cc].map((target) => locale === 'ar' ? target.labelAr || target.label : target.label || target.labelAr).join(locale === 'ar' ? '، ' : ', ')}</bdi></p></ActionDialog>
    </section>
  );
}

function ActionTaskFields({ assignee, dueAt, locale, onDueAtChange, onTitleChange, timeZone, title }: { assignee: CollaborationTarget | null; dueAt: string; locale: Locale; onDueAtChange: (value: string) => void; onTitleChange: (value: string) => void; timeZone: string; title: string }) {
  const t = complaintCommentText[locale];
  return <div className="grid gap-3 border-s-2 border-brand/30 ps-3"><p className="text-sm text-content-muted">{locale === 'ar' ? 'اختر المسؤول من نتائج البحث أعلاه. هو وحده المسؤول عن تنفيذ المهمة.' : 'Choose the assignee from the search results above. They alone are responsible for the task.'}</p><Label className="grid gap-1 text-sm font-medium">{t.actionTitle}<Input onChange={(event) => onTitleChange(event.target.value)} value={title} /></Label><div className="grid gap-1 text-sm"><span className="font-medium">{t.assignee}</span><span>{assignee ? displayTarget(assignee, locale) : missingDisplay(locale)}</span></div><Label className="grid gap-1 text-sm font-medium">{t.dueAt} ({timeZone})<Input lang={locale === 'ar' ? 'ar' : 'en'} onChange={(event) => onDueAtChange(event.target.value)} type="datetime-local" value={dueAt} /></Label></div>;
}

function CommentGroup({ badge, comments, locale, timeZone, title }: { badge: string; comments: StaffComplaintComment[]; locale: Locale; timeZone: string; title: string }) {
  const t = complaintDetailText[locale];
  return <section className="rounded-md border border-line-subtle bg-surface-raised p-3" aria-label={title}><div className="flex flex-wrap items-center justify-between gap-2"><h3 className="text-sm font-semibold">{title}</h3><StatusBadge>{badge}</StatusBadge></div>{comments.length ? <ol className="mt-3 grid gap-2">{comments.map((comment) => <li className="rounded-sm border border-line-subtle bg-surface p-3 text-sm" key={comment.id}><div className="flex flex-wrap items-center justify-between gap-2"><strong><bdi>{locale === 'ar' ? comment.authorNameAr || comment.authorName || t.values.author : comment.authorName || comment.authorNameAr || t.values.author}</bdi></strong><time className="text-content-muted" dateTime={comment.createdAt}>{formatDisplayDate(comment.createdAt, locale, { dateStyle: 'medium', timeStyle: 'short', timeZone })} ({timeZone})</time></div><p className="mt-2 whitespace-pre-wrap break-words">{comment.body}</p>{comment.mentions?.length ? <p className="mt-2 text-content-muted">@ {comment.mentions.map((item) => locale === 'ar' ? item.nameAr || item.name : item.name || item.nameAr).join('، ')}</p> : null}{comment.createdTaskId ? <a className="mt-2 inline-block font-medium text-brand underline-offset-4 hover:underline" href={localeHref(`/tasks/${comment.createdTaskId}`, locale)}>{locale === 'ar' ? 'فتح المهمة المرتبطة' : 'Open linked task'}</a> : null}</li>)}</ol> : <StateBlock className="mt-3" message={t.commentStates.empty} />}</section>;
}
...
function audienceSummary(value: string, mentions: number, cc: number): string { return value.replace('{mentions}', String(mentions)).replace('{cc}', String(cc)); }
