...
import { createAdminRoleAction, updateAdminRolePermissionsAction } from './actions';
...
export default async function AdminRolesPage({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  const state = value === 'success' || value === 'validation' || value === 'conflict' || value === 'error' ? value : undefined;
  const locale = resolveLocale(Array.isArray(params?.locale) ? params?.locale[0] : params?.locale);
  return <AdminRoles createAction={createAdminRoleAction} data={await getAdminRoles()} locale={locale} state={state} updateAction={updateAdminRolePermissionsAction} />;
}
