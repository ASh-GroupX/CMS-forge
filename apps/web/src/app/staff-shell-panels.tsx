    ...
    return (
      <section className="mb-4 rounded-md border border-slate-200 bg-slate-50 p-3" aria-label={t.auth.signedIn}>
        ...
        <form action={logoutStaffAction}>
          ...
          <button className="mt-3 inline-flex rounded-sm border border-slate-300 px-3 py-2 text-sm font-semibold hover:bg-white focus:outline-none focus:ring-2 focus:ring-brand" type="submit">
            {t.auth.logout}
          </button>
        </form>
      </section>
  ...
  return (
    <section className="mb-4 rounded-md border border-slate-200 bg-slate-50 p-3" aria-label={t.auth.loginTitle}>
      ...
      <form action={loginStaffAction} className="mt-3 grid gap-2">
        ...
        <label className="grid gap-1 text-sm font-medium">
          {t.auth.password}
          <input className="rounded-sm border border-slate-300 px-3 py-2" name="password" type="password" autoComplete="current-password" />
        </label>
