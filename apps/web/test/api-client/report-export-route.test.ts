...
test('report export route proxies valid formats with staff cookies', async () => {
  ...
  try {
    globalThis.fetch = async (input, init) => {
      assert.equal(String(input), 'http://api.test/reports/export?format=csv&branchId=branch_report&categoryId=cat_report&departmentId=dept_report&ownerId=usr_report');
    ...
    const response = await GET(new Request('http://web.test/reports/export?format=csv&branchId=branch_report&categoryId=cat_report&departmentId=dept_report&ownerId=usr_report', {
